import type { BurnNoteData, PatientData, TbsaResult, FluidResult, RegionSelection } from './types';
import { calcAgeBand } from './tbsa';
...
BURN ASSESSMENT:
- Total Body Surface Area: ${tbsa.tbsaPct}% (${tbsa.ageBand} age group)
...
export function makeDischargeTeachingNote(data: BurnNoteData): string {
  ...
  let teachingContent = '';
  Object.entries(DISCHARGE_TEACHING_TOPICS).forEach(([key, topic]) => {
...
export function makeBurnProcedureNote(data: BurnNoteData): string {
  const { patient, tbsa, regions, timestamp } = data;
...
- TBSA: ${tbsa.tbsaPct}% (${tbsa.ageBand} calculations)
